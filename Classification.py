...
for t in range(200):
    ...
    if t % 4 == 0:
        ...
        plt.cla()
        prediction = torch.max(out, 1)[1]          #第一个1指的是按行取最大值，第二个[1]表示取最大时的index
        pred_y = prediction.data.numpy().squeeze() #200*1 值全部为0或1，表示预测值
        target_y = y.data.numpy()
        plt.scatter(x.data.numpy()[:, 0], x.data.numpy()[:, 1], c=pred_y, s=100, lw=0, cmap='RdYlGn')
        accuracy = sum(pred_y == target_y)/len(target_y)
        plt.text(1.5, -4, 'Accuracy=%.2f' % accuracy, fontdict={'size': 15, 'color':  'b'})
        plt.pause(0.1)
